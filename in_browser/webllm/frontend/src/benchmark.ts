...
export async function runBenchmark(test: TestParameters): Promise<BenchmarkResult> {
  ...
  try {
    ...
    // We'll move startTime to after model loading is complete
    let modelLoadingStartTime = performance.now();
    // Declare startTime here but initialize it after model loading
    ...
    try {
      ...
      // Now set the startTime after model loading is complete
      startTime = performance.now();
      const modelLoadingTimeMs = startTime - modelLoadingStartTime;
      debug(`Model loading took ${modelLoadingTimeMs.toFixed(2)}ms`);
