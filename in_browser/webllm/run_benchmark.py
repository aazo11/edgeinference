...
def run_webllm_benchmark(test, debug=False, force_visible=False, batch_info=None):
    ...
    Takes a BenchmarkTest object and returns (response, latency_ms, tokens_per_second, time_to_first_token).
    ...
    response, latency_ms, tokens_per_second, time_to_first_token = run_browser_benchmark(
        test, 
        debug=debug,
...
def run_webllm_benchmark_batch(tests: List, debug=False, force_visible=False) -> List[Tuple[str, float, float, float]]:
    """
    ...
    Returns:
        List of tuples, each containing (response, latency_ms, tokens_per_second, time_to_first_token)
    """
...
if __name__ == "__main__":
    ...
    if args.batch:
        ...
        try:
            ...
            # Print the results
            print("\n=== BATCH RESULTS ===")
            for i, (response, latency_ms, tokens_per_second, time_to_first_token) in enumerate(results):
                print(f"\nTest {i+1}: {tests[i].id}")
                print(f"Latency: {latency_ms:.2f} ms")
                print(f"Tokens per second: {tokens_per_second:.2f}")
                print(f"Time to first token: {time_to_first_token:.2f} ms" if time_to_first_token is not None else "Time to first token: N/A")
                print("\n=== RESPONSE ===")
            ...
            with open(results_file, 'w', encoding='utf-8') as f:
                ...
                } for i, (response, latency_ms, tokens_per_second, time_to_first_token) in enumerate(results)], f, indent=2)
        ...
        try:
            ...
            response, latency_ms, tokens_per_second, time_to_first_token = run_webllm_benchmark(
                test, 
                debug=args.debug, 
                force_visible=args.visible
            )
